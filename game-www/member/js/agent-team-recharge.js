$(document).ready(function () {
    ...
    var doSearch = function () {
        initReqData();
        AgentCtrl.request('SEARCH_ACCOUNT_RECHARGE', {
            data: reqData,
            beforeSend: function () {
    ...
    var buildData = function () {
        ...
        if (resData.list.length > 0) {
            $.each(resData.list, function (i, v) {
                ...
                var tpl =
                    '<tr>\
                        <td class="text-center">' + v.billno.substring(12) + '</td>\
                        <td class="text-center">' + v.account + '</td>\
                        <td class="text-center">' + DataFormat.AccountRecharge.method(v.method) + '</td>\
                        ...
                        <td class="text-center">' + DataFormat.AccountRecharge.orderStatus(v.orderStatus) + '</td>\
                        <td class="text-center">' + moment(v.payTime).format('YYYY-MM-DD HH:mm:ss') + '</td>\
