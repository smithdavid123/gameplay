// 订单详情
var LotteryOrderModalSM = function () {
    // 获取内容
...
// 追号详情
var LotteryChaseModalSM = function () {
    // 获取内容
    var getContent = function () {
        var tpl =
            '<div class="modal-table-wrapper lottery-order-details">\
                ...
                <div class="tab-content">\
                    ...
                    <div data-tab="list" class="content-panel">\
                        <table class="table table-bordered table-data">\
                            <thead>\
                                <tr>\
                                    <td width="110px" class="text-center">期号</td>\
                                    <td class="text-center">倍数</td>\
...
// 投注订单
var RecordOrderSM = function () {

    // 声明元素
    var thisPanel = $('[order-type="2"]').find('[data-name="order"]');
    ...
    var doShowDetails = function (billno) {
        GameLotteryCtrl.request('GET_ORDER', {
            ...
            success: function (res) {
                if (res.error == 0) {
                    LotteryOrderModalSM.init(res.data, {
                        onCancel: function () {
                            reload();
                        }
                    });
...
// 追号订单
var RecordChaseSM = function () {

    // 声明元素
    var thisPanel = $('[order-type="2"]').find('[data-name="chase"]');
    ...
    var doShowDetails = function (billno) {
        GameLotteryCtrl.request('GET_CHASE', {
            ...
            success: function (res) {
                if (res.error == 0) {
                    LotteryChaseModalSM.init(res.data, {
                        onCancel: function () {
                            easyPage.reload();
                        }
                    });
