...
from libs.utils import TM, S2T, TMSL, S2TS, TS2S, S2TSL, TimeOffsetS, TimeOffset, PreNow, DiffSecond, F00, F000, after30s
...
def producer():
    ...
    def addQuestTime(args):
        ...
        if not res or res == [] or 'NextOpenTime' not in res:
            ...
            return Timer.setTimeout(addQuestTime, 40, n)
        
        '''时间合理性判断'''
        ntm, seconds = res['NextOpenTime'], [7, 12, 16, 30, 45, 70] 
        if not ntm: ntm = after30s(TM())
        ds = DiffSecond(ntm)
        if ds < 0: ntm = TM()
    ...
    for n in Games:
        ...
        game = Games[n]
        seconds = game.interval * 60 / 6
        param = {'name': n, 'tm': TMSL(), 'issue': ''}
        if n not in set(['t1s60h', 't1s300']): addQuestTime([n])
        if n in {'qumin': 1}: seconds = 4
...
#     if name in API_GAMES:
#         return APIS[API_GAMES[name]](name)
...
# '''****数据爬取后补程序****'''
# def repaireData(name = 't1s300a', count = 100):
#     url = 'http://47.244.4.22:8080/web/lottery/list'
#     param = {'lottery': name, 'num': 1, 'pageSize': count}
#     res = requestUrl(url, param)
#     
#     '''统一数据格式'''
#     init()
#     data = formatDataList(name, res)
#     Games[name].getDataEx(data)
#     Games[name].updateData()
#     
# 
...
#     src = request_url(url)
    ...
    try:
        ...
        if src['latest_num'].strip() == '': return None
...
#     testApi()
#     print rs.hget(KEY.UserInfo, 'test1992')
#     import re
#     print re.match("^(?:(?=.*[A-Z])(?=.*[a-z])).{6,16}$", 'asAaasasxsxasxa')
#     print rs.hget(KEY.UserInfo, 'test1993').get('defaultPasswd', 0)
#     print rs.zcard(KEY.RequestJob)
#     Games[name].getDataEx(data)
#     Games[name].updateData()

#     print getNewData('xjssc')
#     print getNewData('t6s300')
    
    for item in S.values():
        game = item['class']()
        print game.name, rs.hkeys('pre' + game.name), game.openMode, game.openMode == 0
#         if game.openMode != 0: print 'Error:', game.name
