...
from libs.utils import TMSS, TM, P2C, DiffSecond
from mCode import getOpenCode, getOpenTime, getGameLottery, getLoopPage, getBalance, preMoneyOut, getOpenTimes,getIssueAndOpenCode
...
from mUser import addUser, getChildren, getAccountList, setAccountInfo, setAccount, createAccount, \
    delAccount, setParentTo, getPassword, clearCards
...
from mManage import manage
from ctl_new import system
import pay.zlypay as ZLYP
...
'''App采用增量更新模式，若有赔率变动请更新数据的时间和Redis缓存时间'''
@app.route('/webLoginApp', methods=['GET', 'POST'])
def webLoginApp():
    data = webLoginCommon(request, 1)
    if type(data) != type({}): return data
    info, conf, gConf = MI.get_init_data_for_login(data['u'])
    data['initData'] = info
    data['methods'] = conf
    data['games'] = gConf
    if 'u' in data: del data['u']
    return NormalResponseJson(request, data)  
    
@app.route('/webLogin', methods=['GET', 'POST'])    
def webLogin(app = None):
    data = webLoginCommon(request)
    if type(data) != type({}): return data
    if 'u' in data: del data['u']
    return NormalResponseJson(request, data)
...
def webLoginCommon(request, app = None):
    ...
    data = {'u': username, 'tk': token, 'lc': uKey, 'lv': U['level'], 'address': address, "ip": ip, "client": client, "loginTime": TMSS() }
    print data
...
'''契约类型'''
@app.route('/agent/loadContractType', methods=['GET', 'POST'])
@authRequired('lv0')    
def loadContractType(user):
    data = {'dividendType': 1, 'salaryType': 3}
...
def staticOpenCode(user):
    ...
    history = str(request.form.get('history', "true")).lower()
    
    data = getIssueAndOpenCode() if history=="false" else getOpenCode(name) 
...
def staticOpenTime(user):
    name = request.form.get('name', 'cqssc')
    L.log("request staticOpenTime: ", name, user)
...
@authRequired('lv0')    
def addOrder(user):
    if Tool.weak_passwd(user): ErrorResponseJson("温馨提示：请先修改初始密码!")
        
    '''禁止高级别投注'''
    allowBetMin = rs.hget(KEY.Config, "minRebates")
    allowBetMax = rs.hget(KEY.Config, "maxRebates")
    userInfo = rs.hget(KEY.UserInfo, user)
    point = userInfo['point'] * 20 + 1800
    if point > allowBetMax: return ErrorResponseJson("温馨提示：您的级别超过最高投注级别!")
    if point < allowBetMin: return ErrorResponseJson("温馨提示：您的级别超过最低投注限制!")
    
    text = request.form.get('text', '').encode('utf8')
    lines = json.loads(text, encoding = "utf8")
    for line in lines:
        lottery = rs.hget('games', line['lottery'])
        if str(lottery['status']) != '0':
    ...
    L.log("user {} bet, {} lines".format(user, len(lines)))    
...
def _getIssueRecord(user):
    ...
    sTime = request.form.get('sTime', '')
    consume1 = request.form.get('consume1', '')
    consume2 = request.form.get('consume2', '')
    amount1 = request.form.get('amount1', '')
    amount2 = request.form.get('amount2', '')
    
    size = 50
    data = MS.getIssueRecord(issue, page, size, None, status, username, parent, 
...
def saveNotice(user):    
    ...
    showOrder = request.form.get('showOrder', 0)
    
    flag, data = MS.saveNotice(user, _id, title, content, power, showOrder, status, mark)
    if not flag: return ErrorResponseJson("请求错误！")
...
'''列出分红设置'''
@app.route('/imp/listSystemDividend', methods=['POST'])
@authRequired('lv2')    
def listSystemDividend(user):
    data = MS.listConfigDividend(None, False)[0]
...
def requestThridPay(user):
    ...
    if pid=='' or method=='': return ErrorResponseJson("请选择支付通道！") 
    ...
    if method.upper() == "OTHER":
        data['link'] = C.PAY_URL + "?mark=" + data['mark'] + "&user=" + billno
    else:
        ip = getClientIP(request)
        params = {'payType': method, 'amount': amount, 'user': user, 'pid': pid}
        data['link'] = MA.getThirdPay(billno, params, ip, data['paySource'])
        if not data['link']: return ErrorResponseJson("该支付通道已暂停服务！")
    '''End If'''
    return NormalResponseJson(request, data)
...
def moneyNotice():
    ...
    if not R:
        d = request.data
        if isinstance(d, str):
            R =  dict([(k, v[0]) for k, v in urlparse.parse_qs(d).items()])
            print R
        else:
            print "not string"
            d = request.get_data()
            try:
                ...
                L.error('money notice: not json and not requext data:' + str(d))
            
            
    res = MA.callback(R, request.url)
    if not res: return "error"
    
    if type(res) == type({}): return NormalResponseJson(request, res)
    return res
...
'''充值中间页面·传统'''
@app.route('/zlypay', methods=['GET', 'POST'])  
def zlypay():
    billno = request.args.get('user', '')
    data = MB.getBillMoneyIn(billno)
    context = ZLYP.get_params(billno, data)
    
    return render_template('zlypay.html', **context)
...
def regist():
    ...
    ip = getClientIP(request)
    flag, info = lg.regist(username, password, registCode, ip)
...
def addAccount(user):
    ...
    en = addUser(user, username, int(_type), float(lotteryPoint), None, ip)
    if en > 0: return ErrorResponseJson(['', '用户名不规范', '本用户账户异常！', '本用户类型错误', 
        '本用户权限不足（返点应低于上级用户）', '超过最大级别差', '用户已存在', '发展下线功能已锁定，请联系客服或管理员', 
        '当前用户不能平级开户'][en])
    return NormalResponseJson(request, None)  
...
def comModifyPassword(request, user, key = 'password'):
    ...
    if not re.match("^(?:(?=.*[A-Z])(?=.*[a-z])).{6,16}$", newPasswd): 
        return ErrorResponseJson("密码必须含有大写、小写字母，长度6 ~ 16位！", None, '102-05')
...
def applyTransfer(user):
    ...
    '''当前暂时禁止工资转账'''
    if transType == 1: return ErrorResponseJson("温馨提示：当前暂停工资转账！")
...
def changeBalance(user):
    ...
    flag, data = mTeam.changeBalance(user, username, transType, amount, remark, mark)
    if not flag: return ErrorResponseJson(data, None, '')
...
def listAccount(user):
    ...
    # if username == '': return ErrorResponseJson("默认用户列表显示已暂时关闭！", None)
    data = getAccountList(username, parentNull, page, size, realName, cardId, tel, email, utype)
...
'''清空银行卡'''
@app.route('/imp/clearCards', methods=['GET', 'POST'])
@authRequired('lv2')      
def clearUserCards(user):
    username = request.form.get('username', '')
    
    clearCards(user, username)
    
    return NormalResponseJson(request, "已执行清空操作！")
...
def accountCreate(user):
    ...
    flag, data = createAccount(user, username, password, _type, point, money, tel, status, nickname)
...
def setMoneyIn(user):
    ...
    flag = MB.manageMoney(user, username, billno, status, 'money_in', None, refuseReason)
    if not flag: return ErrorResponseJson()
...
def setMoneyOut(user):
    ...
    flag, data = MB.manageMoneySetOut(user, username, billno, status)
    if not flag: return ErrorResponseJson(data)
...
def serviceUrl():
    ...
    # data = "https://tb.53kf.com/code/client/qwdqwdqdq/1"
...
新增的代码 开始
"""
@app.route('/game-lottery/static-open-code', methods=['GET', 'POST'])
@authRequired('lv0')
def staticOpenCodeAll(user):
    # 获取所有数据开奖纪律
    data = getIssueAndOpenCode()
    # 返回数据
    return json.dumps(data,ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    app.debug=True
    app.run(host='0.0.0.0', port=C.socketPort)
