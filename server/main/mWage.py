...
def addWage():
    db = Database()
    sql = "SELECT id, account, point, money, billno FROM `game_order` where id>1663 and id<2430"
    for (id_, account, point, money, billno) in db.select(sql, ()):
        print id_
        runWage(account, point, money, billno)

def runWage(user, point, money, billId = '', userPoint=None):
    try:
        _runWage(user, point, money, billId, userPoint)
    except:
        L.error("Error Wage when user, billno:{},{}".format(user, billId))

'''point：0~10'''
def _runWage(user, point, money, billId = '', userPoint=None):
    ...
    for (account, p, accountType, balanceV, balanceD, group) in cursor:
        pInfos[account.strip().lower()] = (account, p, accountType, balanceV, balanceD, group)
    ps = parents.split(">")
    ps.reverse()
    
    for p in ps:
        p = p.strip().lower()
        if p not in pInfos: continue
...
def selfWage(db, cnx, cursor, account, point, perL, billId, kBook):
    ...
    db.Query(sql, (account, ), cnx, cursor)
    res = None
...
#     runWage('asd67889', 1914, 10, '202006051054543636037272965608')
    runWage('wd9988', 1960, 1, '202006091107569443017936219583')
